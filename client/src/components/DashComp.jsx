...
export default function DashComp() {
  ...
  if (currentUser?.role !== "admin") {
    return (
      <div className="flex-1 p-4 text-center text-red-500">
        Access Denied. Only admins can view dashboard statistics.
      </div>
    );
  }
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch(`/api/v1/comment/getAllUsersComments?limit=5`);
        const data = await res.json();
        if (res.ok) {
    ...
    if (currentUser?.role === "admin") { // 🔥 role check
      fetchUsers();
      fetchComments();
      fetchPosts();
    }
        ...
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800">
          ...
          <Table hoverable>
            <Table.Head>
              ...
              <Table.HeadCell>Role</Table.HeadCell> {/* 🔥 NEW */}
            </Table.Head>
        ...
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800">
          ...
          <Table hoverable>
            <Table.Head>
              ...
              <Table.HeadCell>Status</Table.HeadCell> {/* 🔥 NEW */}
            </Table.Head>
